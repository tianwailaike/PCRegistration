...
IOManager::ReadObj(const char *filename,
  ...
  while (!f.eof()) {
    ...
    sscanf(str, "%s %*s", ch);
    if (strcmp(ch, "v") == 0) {
      //only for .obj generated from meshlab
      sscanf(str, "%s %f %f %f %f %f %f", ch, &x, &y, &z, &r, &g, &b); 
      //sscanf(str, "%s %f %f %f", ch, &x, &y, &z);
      v.emplace_back(x, y, z);
      typename Point3D::VectorType rgb((float)r*255,(float)g*255 ,(float)b*255);
      v[v.size() - 1].set_rgb(rgb);
